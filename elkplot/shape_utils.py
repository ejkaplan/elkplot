...
def _repeat_lines_single(
    drawing: shapely.MultiLineString | shapely.LinearRing | shapely.LineString, n: int
) -> shapely.MultiLineString:
    out = []
    for line in shapely.get_parts(drawing):
        coord_list = list(line.coords)
        new_coords = coord_list.copy()
        for i in range(1, n):
            new_coords += coord_list[-2::-1] if i % 2 == 1 else coord_list[1::]
...
def repeat_lines(drawing: T, n: int = 2) -> T:
    """
    Makes the axidraw go over each line multiple times by extending every line with the same line in the reverse order.
    Ensures that you get nice dark lines by retracing them multiple times.
    Args:
        drawing: The drawing to repeat
        n: How many times each line should be drawn over. (n=1 leaves the drawing unchanged.)

    Returns:
        The updated drawing with all the lines extended to cover the same path n times
    """
    if n == 1:
        return drawing
    elif isinstance(
        drawing, (shapely.MultiLineString, shapely.LinearRing, shapely.LineString)
    ):
        return _repeat_lines_single(drawing, n)
    else:
        return shapely.GeometryCollection(
            [_repeat_lines_single(layer, n) for layer in shapely.get_parts(drawing)]
        )
